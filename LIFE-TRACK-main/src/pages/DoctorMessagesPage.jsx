...
export function DoctorMessagesPage() {
  ...
  const {
    ...
    sendMessage,
    formatContactTime,
  } = useDirectChat({ roleFilter: "patient" });
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      ...
      <div className="min-h-0 rounded-3xl border border-slate-200 bg-white shadow-sm flex lg:h-[calc(100vh-180px)] flex-1 overflow-hidden">
              ...
              {contacts.map((contact) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl flex gap-3 transition-all duration-200 cursor-pointer text-left ${isActive ? "bg-sky-50 border border-primary/20 shadow-sm" : "hover:bg-slate-50 border border-transparent"}`}
                    type="button"
                  >
        ...
        <section className={["min-h-0 flex-1 flex flex-col bg-surface-container-lowest", mobileTab === "chat" ? "flex" : "hidden lg:flex"].join(" ")}>
          <div className="h-20 flex items-center justify-between px-8 bg-white border-b border-slate-100 shrink-0">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <h3 className="font-bold text-on-surface truncate">{activeContact?.name || "Chọn bệnh nhân để bắt đầu"}</h3>
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-8 space-y-6 no-scrollbar">
            ...
            {!loadingConversation && !activeContact && (
              <div className="text-center text-sm text-slate-500">Chọn một bệnh nhân trong danh sách để bắt đầu nhắn tin.</div>
            )}
              ...
              if (!isDoctor) {
                return (
                  <div key={message.message_id} className="flex gap-4 max-w-2xl">
                    <ImageWithFallback alt={activeContact?.name || "Bệnh nhân"} className="w-8 h-8 rounded-full self-end border border-slate-200" src={getUserAvatar(activeContact)} />
          ...
          <div className="p-6 bg-white border-t border-slate-100 shrink-0">
            ...
            <div className="flex items-center gap-3 bg-slate-50/80 rounded-2xl p-2 pl-4 border border-slate-100 focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/10 transition-all">
              <input
                ...
                placeholder={activeContact ? "Viết tin nhắn cho bệnh nhân..." : "Chọn bệnh nhân để bắt đầu trò chuyện"}
                type="text"
                value={draftMessage}
          ...
          <div className="p-6 space-y-8">
            <div>
              ...
              <div className="bg-white p-5 rounded-2xl shadow-sm relative overflow-hidden border border-slate-100">
                <div className="flex items-center gap-3">
                  <ImageWithFallback alt={activeContact?.name || "Bệnh nhân"} className="w-12 h-12 rounded-full border border-slate-200" src={getUserAvatar(activeContact)} />
                  <div>
                    <p className="font-bold text-slate-800">{activeContact?.name || "Chưa chọn bệnh nhân"}</p>
                ...
                <div className="mt-4 grid gap-3">
                  <div className="rounded-xl bg-slate-50 px-4 py-3">
                    ...
                    <p className="mt-1 text-sm font-bold text-slate-700">{activeContact?.roleLabel || "Bệnh nhân"}</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 px-4 py-3">
                  ...
                  <div className="rounded-xl bg-slate-50 px-4 py-3">
                    <p className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Tin nhắn gần nhất</p>
                    <p className="mt-1 text-sm font-bold text-slate-700">{formatContactTime(activeContact?.last_message_at) || "Chưa có"}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <button
                  onClick={() => activeContact && navigate(`/doctor/live?patientId=${activeContact.user_id}`)}
                  className="w-full flex items-center justify-between p-4 bg-primary text-white rounded-xl shadow-md hover:scale-[0.98] transition-transform disabled:opacity-50 disabled:hover:scale-100"
                  disabled={!activeContact}
                  type="button"
                >
                  <span className="font-bold text-sm">Theo dõi trực tiếp</span>
                  <span className="material-symbols-outlined text-lg">monitor_heart</span>
                </button>
                <button onClick={() => navigate('/doctor/appointments')} className="w-full flex items-center justify-between p-4 bg-white text-on-surface border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors" type="button">
                  <span className="font-bold text-sm">Lên lịch khám</span>
                  <span className="material-symbols-outlined text-lg">event_repeat</span>
                </button>
                <button onClick={() => navigate('/doctor/patients')} className="w-full flex items-center justify-between p-4 bg-white text-on-surface border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors" type="button">
            ...
            <div>
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Lưu ý</h2>
              <div className="p-4 bg-white/50 border border-dashed border-slate-300 rounded-xl text-xs text-slate-500 italic leading-relaxed">
                Chat direct chỉ hỗ trợ văn bản. ECG của bệnh nhân được xem riêng ở màn "Theo dõi trực tiếp".
              </div>
            </div>
          </div>
        </section>
